...
TYPED_TEST(darray_trival_tests, assign_with_forward_iterator_with_enough_capacity)
{
    ...
    for (int assign_size = 0; assign_size <= max_assign_size; assign_size += step)
    {
        ...
        expu::seq_iter assign_first(-assign_size), assign_last(0);
        arr.assign(assign_first, assign_last);

        ASSERT_TRUE(is_darray_valid(arr))
            << "Failed trying to assign range: [" << *assign_first << ", " << *assign_last << ")";
        ASSERT_TRUE(is_equal(arr, assign_first, assign_last))
            << "Failed trying to assign range: [" << *assign_first << ", " << *assign_last << ")";
...
void _insert_iterator_test_common(int initial_size, int insert_size, size_t capacity, int number_of_tests, Callable pre_check)
{
    ...
    for (auto insert_at = init_first; insert_at != (init_last + step); insert_at += step)
    {
        ...
        auto failure_message = std::stringstream() << "Failed trying to insert at index:" << at;
