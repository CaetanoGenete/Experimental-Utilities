...
#include "smm/int_seq.hpp"
#include "smm/iterators/sorting.hpp"
...
namespace smm_tests {
    ...
    consteval auto _bubble_sort_int_seq(smm::int_seq<IntType, Seq...>)  
    {
        constexpr size_t seq_size = sizeof...(Seq);

        //Abuse lambdas to sort 
        constexpr auto sorted_arr = [](std::array<IntType, seq_size> arr) {
            smm::bubble_sort(arr.begin(), arr.end());
            return arr;
        }(std::array<IntType, seq_size>{Seq...});

        //Templated lambda to expand sorted_arr
        return []<size_t ... Indicies>(smm::int_seq<size_t, Indicies...>) {
            return smm::int_seq<IntType, sorted_arr[Indicies]...>{};
        }(smm::make_int_seq<size_t, seq_size>{});
    }
