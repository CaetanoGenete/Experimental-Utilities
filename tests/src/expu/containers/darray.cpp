...
namespace smm_tests {
    ...
    public:
        ...
        constexpr static bool populate_darray(expu::darray<counted_int, counted_allocator<Alloc>>& arr, InIt begin, InIt end)
        {
        ...
        constexpr static bool darray_reserve(expu::darray<counted_int, counted_allocator<Alloc>>& arr, size_t capacity) 
        {
            size_t init_size = arr.size();
    ...
    TYPED_TEST(darray_tests, u_emplace_back_nogrow)
    {
        ...
        {
            expu::darray<TestFixture::counted_int, alloc> test;
    ...
    TYPED_TEST(darray_tests, move_construct) 
    {
        ...
        constexpr size_t test_size = 100000;
        {
            expu::darray<TestFixture::counted_int, alloc> to_be_moved;
            to_be_moved.reserve(test_size);

            ASSERT_TRUE(TestFixture::populate_darray(to_be_moved, intit<int>(0), intit<int>(test_size)));

            expu::darray<TestFixture::counted_int, alloc> moved(std::move(to_be_moved));
    ...
    TYPED_TEST(darray_tests, assign_case_1) {
        ...
        expu::darray<TestFixture::counted_int, alloc> temp_cont;
